<template>
  <div>
    ...
    <el-table :data="tableData" style="margin-bottom:15px;">
      ...
      <el-table-column label="总张数" prop="cardNum"></el-table-column>
    ...
    <el-dialog title="详情记录" :visible.sync="showDialog" width="750px" :close-on-click-modal="false" :close-on-press-escape="false" @closed="handleCloseDialog">
      <el-table :data="dTableData" style="margin-bottom:15px;">
        ...
        <el-table-column label="券名称" prop="cardName"></el-table-column>
      </el-table>
      <!-- 分页 -->
...
<script>
import { getGroupSendRecord, getSendCardRecord } from '@/api/vipManage';
...
export default {
  ...
  methods: {
    ...
    handleGetTableData(fn) {
      getGroupSendRecord({currentPage: this.page, name: this.data.name }).then(res => {
    ...
    handleGetDTableData() {
      getSendCardRecord({currentPage: this.dPage, sendId: this.id }).then(res => {
