...
<script>
import { openCardQRcode } from '@/api/shopmanage';
...
export default {
  ...
  methods: {
    handlerGetQRCode() {
      this.url = '';
      openCardQRcode({ refresh: true, staffId: this.$store.getters.staffId }).then(res => {
      ...
      if (this.url) {
        html2canvas(this.$refs["codeImage"]).then(canvas => {
          if (window.navigator.msSaveOrOpenBlob) {
            ...
            window.navigator.msSaveOrOpenBlob(blob, '会员开卡二维码.png');
          } else {
            let link = document.createElement('a');
            link.style.display = 'none';
            link.href = canvas.toDataURL('image/png');
            link.setAttribute('download', '会员开卡二维码');
