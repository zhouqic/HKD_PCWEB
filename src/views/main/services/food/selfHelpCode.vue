...
<script>
import { getMallOrderQRcord } from '@/api/services';
...
export default {
  ...
  methods: {
    handlerGetQRCode(flag) {
      this.url = '';
      getMallOrderQRcord({ isRefresh: flag }).then(res => {
      ...
      if (this.url) {
        html2canvas(this.$refs["codeImage"]).then(canvas => {
          if (window.navigator.msSaveOrOpenBlob) {
            ...
            window.navigator.msSaveOrOpenBlob(blob, '自助点餐二维码');
          } else {
            let link = document.createElement('a');
            link.style.display = 'none';
            link.href = canvas.toDataURL('image/png');
            link.setAttribute('download', '自助点餐二维码');
