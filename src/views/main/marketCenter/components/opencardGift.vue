...
<div class="main-content">
  ...
  <el-table :data="tableData" style="margin-bottom: 15px;">
    ...
    <el-table-column label="优惠券总值">
      <template slot-scope="scope">
        {{ parseFloat(scope.row.cardPrice) ? (scope.row.cardPrice + "元") : '无' }}
      </template>
    </el-table-column>

    <el-table-column label="积分">
      <template slot-scope="scope">
        <span>{{ parseFloat(scope.row.pointPrice) ? scope.row.pointPrice : '无' }}</span>
      </template>
    </el-table-column>

    <el-table-column label="现金红包">
      <template slot-scope="scope">
        {{ parseFloat(scope.row.redPackPrice) ? (scope.row.redPackPrice + "元") : '无' }}
      </template>
    </el-table-column>
  ...
  <!-- 添加新人礼 -->
  <el-dialog :title="id ? '礼包详情' : '添加礼包'" :visible.sync="showDialog" width="800px" @closed="showDialog=false;id='';" :close-on-click-modal="false" :close-on-press-escape="false">
    <!-- 创建活动 -->
    <opencard-form ref="opencardForm" v-if="showDialog && !id" :type="0"></opencard-form>
    <!-- 详情 -->
    <opencard-detail ref="opencardDetail" v-if="id" :id="id" :type="0"></opencard-detail>
...
import opencardDetail from './opencardDetail';
  ...
  export default {
    ...
    methods: {
      ...
      handleGetTableData() {
        getRfNewGiftsByCreaterId({ "pageNo": this.page, "status": this.status, "type": 0 }).then(res => {
      ...
      handleOpenDialog() {
        getRfNewGiftsByCreaterId({ "pageNo": this.page, "status": '0', "type": 0 }).then(res => {
